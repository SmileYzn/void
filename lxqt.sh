...
# Pacotes base
sudo xbps-install -Syu 7zip 7zip-unrar alsa-firmware alsa-utils bash-completion blueman fastfetch ffmpeg ffmpegthumbnailer git man nano pipewire power-profiles-daemon powertop unzip unrar xz zip

# Fontes Adwaita
sudo xbps-install -Syu adwaita-fonts
...
# XDG Utils
sudo xbps-install -Syu xdg-user-dirs xdg-user-dirs-gtk xdg-desktop-portal xdg-desktop-portal-lxqt xdg-utils

# XORG
sudo xbps-install -Syu numlockx xiccd
...
# Extras
sudo xbps-install -Syu featherpad kvantum lm_sensors lximage-qt network-manager-applet pavucontrol-qt qalculate-qt vlc
